...
function draftSummary(draft: SavedOfferDraft): {
	...
	const orderContext =
		payload && isRecord(payload.orderContext) ? payload.orderContext : null;
	...
	const persons =
		typeof personsRaw === "number" &&
		Number.isFinite(personsRaw) &&
		personsRaw >= 0
				...
				<div className="mt-4 space-y-3">
					...
					{loadState === "loaded" && drafts.length === 0 ? (
						<p className="text-sm text-muted">
							Keine gespeicherten Entwürfe vorhanden.
						</p>
					) : null}
					{drafts.map((draft) => {
