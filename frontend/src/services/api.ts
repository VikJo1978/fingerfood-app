...
export async function fetchItems(
	...
	if (!res.ok)
		throw new Error(`Artikel konnten nicht geladen werden (${res.status})`);
...
export async function fetchSections(): Promise<string[]> {
	...
	if (!res.ok)
		throw new Error(`Bereiche konnten nicht geladen werden (${res.status})`);
...
export function offerDraftToCalculateBody(
	draft: OfferDraft,
): OfferCalculateBody {
	return {
...
export async function calculateOffer(
	body: OfferCalculateBody,
): Promise<OfferCalculateResponse> {
