...
export function HomePage() {
	...
	const [importedInquiryId, setImportedInquiryId] = useState<string | null>(
		null,
	);
	...
	const [sessionStatus, setSessionStatus] =
		useState<SessionBootstrapStatus>("loading");
	...
	const clampPersons = (n: number) =>
		Math.min(5000, Math.max(0, Math.round(n) || 0));
	...
	const handlePrepareOffer = useCallback(
		(transfer: InquiryToConfiguratorTransferV1) => {
			...
			setOfferDraft((d) => ({
				...d,
				persons:
					planning.persons === null
						? d.persons
						: clampPersons(planning.persons),
				...
				orderContext: {
					...
					billingAddress: pre.billingAddress.trim()
						? pre.billingAddress.trim()
						: undefined,
	...
	useEffect(() => {
		...
		async function bootstrapHandoff(): Promise<void> {
			if (sessionStatus === "disabled") {
				...
				if (result.present) {
					if (result.handoff === null) {
						setHandoffError(
							"Anfragedaten konnten nicht sicher übernommen werden.",
						);
						return;
					}
			...
			if (window.location.hash.startsWith(CORE_INQUIRY_FRAGMENT_PREFIX)) {
				...
				setHandoffError(
					"Unsigned Core-Handoff wird im Mitarbeiter-Modus abgewiesen.",
				);
				return;
			}
			...
			if (sessionStatus !== "authenticated" || !sessionCsrfReady) {
				setHandoffError(
					sessionNotice ?? "Core-Handoff konnte nicht autorisiert werden.",
				);
				return;
			}
	...
	const onAddLine = (
		...
		const hasSurcharge =
			item.surcharge_amount != null && !!item.surcharge_label;
	...
	const onLineQty = (lineId: string, q: number) => {
		setOfferDraft((d) => ({
			...
			lines: d.lines.map((l) =>
				l.lineId === lineId
					? { ...l, quantity: Math.max(1, Math.round(q)) }
					: l,
			),
		}));
	};
	...
	const exportPayload = () => {
		const lines = offerDraft.lines.map((l) => {
			...
			return {
				...
				quantity: l.quantity,
				lineTotal,
				...(l.customizationNote?.trim()
					? { customizationNote: l.customizationNote.trim() }
					: {}),
		...
		return {
			meta: {
				...
				totalBudgetEUR: offerDraft.budgetEnabled
					? offerDraft.totalBudget
					: null,
	...
	const onPrepareInCore = useCallback(async () => {
		...
		if (!offerDraft.orderContext.eventDate) {
			setPrepareStatus("error");
			setPrepareMessage(
				"Bitte zuerst ein Eventdatum im Auftragskontext setzen.",
			);
			return;
		}
		...
		if (returnLogistics?.mode === "SAME_DAY") {
			...
			if (pickupStart && pickupEnd && pickupStart >= pickupEnd) {
				setPrepareStatus("error");
				setPrepareMessage(
					"Beim strukturierten Rückholfenster muss Von vor Bis liegen.",
				);
				return;
			}
		}
		...
		if (
			...
			setPrepareStatus("error");
			setPrepareMessage(
				"Pauschalen pro Person benötigen eine gültige Personenzahl.",
			);
			return;
		}
		if (
			offerDraft.chargesDefinition.dishware.additionalLines.some(
				(line) =>
					line.description.trim() === "" || line.description.length > 500,
			)
		) {
			setPrepareStatus("error");
			setPrepareMessage(
				"Bitte zusätzliche Geschirrpositionen vollständig ausfüllen.",
			);
			return;
		}
	...
	const heroTitle =
		inquiryEventType ||
		offerDraft.orderContext.companyName.trim() ||
		"Catering-Anfrage";
	const heroFacts = [
		offerDraft.orderContext.eventDate
			? formatDateDe(offerDraft.orderContext.eventDate)
			: "",
			...
			<div className="space-y-5">
				<div className="grid gap-[22px] lg:h-[calc(100dvh-110px)] lg:grid-cols-[minmax(0,1.35fr)_minmax(420px,1fr)] lg:overflow-hidden">
					<div className="grid content-start gap-[22px] lg:h-full lg:overflow-y-auto lg:overscroll-contain lg:pr-1">
						...
						{loadError ? (
							<WarningBanner tone="danger" message={loadError} />
						) : null}

						{sessionNotice ? <WarningBanner message={sessionNotice} /> : null}

						{addItemError ? (
							<WarningBanner tone="danger" message={addItemError} />
						) : null}
						...
						<TopControls
							...
							budgetEnabled={offerDraft.budgetEnabled}
							onBudgetEnabledChange={(v) =>
								setOfferDraft((d) => ({ ...d, budgetEnabled: v }))
							}
							...
							onBudgetTypeChange={(v) =>
								setOfferDraft((d) => ({ ...d, budgetType: v }))
							}
							budgetBasis={offerDraft.budgetBasis}
							onBudgetBasisChange={(v) =>
								setOfferDraft((d) => ({ ...d, budgetBasis: v }))
							}
							budgetScope={offerDraft.budgetScope}
							onBudgetScopeChange={(v) =>
								setOfferDraft((d) => ({ ...d, budgetScope: v }))
							}
						/>
				...
				<footer className="border-t border-line pt-6 text-center text-xs text-muted">
					Keine Buchung — nur Orientierung. Summen: {formatCurrency(subtotal)} ·
					Status: Entwurf
				</footer>
			</div>
