...
function isOfferLine(value: unknown): value is OfferLine {
	...
	if (
		value.customizationNote !== undefined &&
		typeof value.customizationNote !== "string"
	) {
		return false;
	}
...
function isReturnLogisticsDefinition(value: unknown): boolean {
	...
	if (
		!isCanonicalOptionalWindow(
			value.pickupWindowStartLocal,
			value.pickupWindowEndLocal,
		)
	) {
		return false;
	}
