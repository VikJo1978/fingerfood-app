...
describe("normalizeRestoredOfferDraft", () => {
	...
	it("normalizes legacy budget fields and later charge substructures", () => {
		const source = createInitialOfferDraft() as unknown as Record<
			string,
			unknown
		>;
		...
		expect(restored?.budgetScope).toBe("positions_only");
		expect(
			restored?.chargesDefinition.delivery.fulfillment?.fulfillmentMode,
		).toBe("UNKNOWN");
		expect(restored?.chargesDefinition.returnLogistics?.mode).toBe(
			"NEXT_WORKING_DAY",
		);
	});
